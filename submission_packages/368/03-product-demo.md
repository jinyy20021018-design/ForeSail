...
ForeSail 把每笔货当作一个持续存在的 trade Case：从合同、订舱确认和信用证中抽取并确认贸易要素，映射 Incoterms 责任与义务/期限日历，持续扫描外部事件并筛选真正触及本单的风险，再由确定性可审计引擎完成核心风险评分与分类、暴露映射、责任归属和 Case 状态流转。Gemini 用于文档抽取、运行摘要、相关性因子候选、动作候选和处置方案草稿；结构化生成结果经过关键字段、枚举、日期格式和已知 ID 的代码校验或清洗，并由用户确认。Gemini 生成的建议行动日期或方案估算成本不会改写已确认的交易金额、合同期限或核心风险决策。
...
- 5. 运行 Agent 监控 / 生成动作：读取带来源的可复现演示事件并调用实时外部连接器 → 相关性判定 → 暴露映射 → 义务/期限映射 → 生成动作与对外草稿。
- 6. 复核并确认推荐动作。
...
- 输入：暴露、义务与期限。
- 系统处理：确定性 exposure、obligation/deadline 与 status 服务产出核心风险和状态结果；Gemini 基于已确认事实与确定性结果生成动作候选和处置方案草稿。系统要求 JSON 对象输出，并对关键字段、优先级与责任方枚举、日期格式、方案数量/类型以及 hazard、obligation、exposure、action 和 gap 等已知 ID 进行校验或清洗，结果仍须用户确认。
- 输出：带证据的处置建议、可确认的动作候选、可发送对外草稿与三类 treatment plan 草稿。
...
4. 按引导巡览 Overview 与信号来源：Overview（Route alerts 点卡片联动地图、Route risk map 点风险看归属 note、Shipment facts、红/绿 Exposure flags）→ External Events（全球航运走廊状态板 +『Search Real External Events』实时拉真实新闻源（gCaptain、Maritime Executive 等，事件带可点的真实来源链接）与天气）→ Risks & Obligations（截止期倒计时矩阵 + 相关性打分 + CIF 下买/卖方归属）。
   - 预期结果：看到风险如何从实时连接器信号或带来源的可复现演示事件 → 打分 → 落到本单的义务 / 期限 / 责任方；地图与走廊状态板随之高亮本航线。
...
- 包含创新点说明：否
- 包含本地化说明：否
- 用户问题：未标注具体时间点；视频中提到：外贸公司缺少持续监测外部风险的工具。
- Demo：未提供
- Google技术：未提供
- 创新点：未提供
- 本地化：未提供
